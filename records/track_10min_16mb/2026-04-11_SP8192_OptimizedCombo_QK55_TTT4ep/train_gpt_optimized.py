...
class DiffusionDenoiser(nn.Module):
	def __init__(self,dim,hidden_dim,steps,beta_start,beta_end,denoise_strength=0.85):super().__init__();self.dim=dim;self.hidden_dim=hidden_dim;self.steps=steps;self.denoise_strength=nn.Parameter(torch.tensor(denoise_strength,dtype=torch.float32));self.register_buffer('betas',torch.linspace(beta_start,beta_end,steps,dtype=torch.float32));alphas=1.-self.betas;self.register_buffer('alphas_cumprod',torch.cumprod(alphas,dim=0));self.net1=nn.Sequential(nn.Linear(dim+1,hidden_dim),nn.LeakyReLU(0.5),nn.Linear(hidden_dim,hidden_dim),nn.LeakyReLU(0.5),nn.Linear(hidden_dim,dim));self.net2=nn.Sequential(nn.Linear(dim,hidden_dim//2),nn.LeakyReLU(0.5),nn.Linear(hidden_dim//2,dim))
	def get_alpha_bar(self,t):return self.alphas_cumprod[t].view(-1,1,1)
	def forward(self,x_noisy,t_embed,noisy_scale=1.):t_norm=t_embed.float()/self.steps;cond=torch.cat((x_noisy,t_norm.unsqueeze(-1).expand(-1,-1,self.dim)),dim=-1);pred_noise=self.net1(cond);pred_clean=self.net2(x_noisy);denoised=x_noisy-pred_noise*noisy_scale;return denoised*self.denoise_strength.to(dtype=denoised.dtype)+x_noisy*(1.-self.denoise_strength.to(dtype=x_noisy.dtype))
	def sample_t(self,batch_size,device):return torch.randint(0,self.steps,(batch_size,),device=device,dtype=torch.long)
	def add_noise(self,x,t):alpha_bar=self.get_alpha_bar(t).to(x.device);noise=torch.randn_like(x);return alpha_bar.sqrt()*x+(1.-alpha_bar).sqrt()*noise,noise
	def train_step(self,x_embed):B,T,D=x_embed.shape;t=self.sample_t(B*T,x_embed.device);x_flat=x_embed.reshape(B*T,D);x_noisy,noise=self.add_noise(x_flat,t);t_expanded=t.unsqueeze(-1).expand(-1,D);loss=F.mse_loss(self.forward(x_noisy,t,1.).reshape(B,T,D),x_embed);return loss
...
class GPT(nn.Module):
	def __init__(self,h):
		...
		self.diffusion_enabled=h.diffusion_enabled
		if self.diffusion_enabled:self.diffusion=DiffusionDenoiser(h.model_dim,h.diffusion_hidden_dim,h.diffusion_steps,h.diffusion_beta_start,h.diffusion_beta_end,h.diffusion_denoise_strength)
		else:self.diffusion=None
		self._init_weights()
	...
	def forward_logits(self,input_ids):
		...
		if self.diffusion_enabled and self.training:t=self.diffusion.sample_t(x.shape[0]*x.shape[1],x.device);x_noisy,noise=self.diffusion.add_noise(x.reshape(-1,x.shape[-1]),t);x_denoised=self.diffusion(x_noisy,t).reshape(x.shape);x=x_denoised
	...
	def forward(self,input_ids,target_ids):
		...
		ce_loss=F.cross_entropy(logits.reshape(-1,logits.size(-1)).float(),target_ids.reshape(-1),reduction='mean')
		if self.diffusion_enabled and self.training and self.diffusion is not None:
			with torch.no_grad():
				x_emb=self.tok_emb(input_ids)
				x_emb=F.rms_norm(x_emb,(x_emb.size(-1),))
				if self.embed_proj is not None:x_emb=self.embed_proj(x_emb)
			diff_loss=self.diffusion.train_step(x_emb.detach())
			return ce_loss+0.1*diff_loss
		return ce_loss
...
class Muon(torch.optim.Optimizer):
	...
	def step(self,closure=None):
		...
		return loss
CONTROL_TENSOR_NAME_PATTERNS=tuple(pattern for pattern in os.environ.get('CONTROL_TENSOR_NAME_PATTERNS','attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,skip_gates,diffusion').split(',')if pattern)
class Optimizers:
	def __init__(self,h,base_model):
		...
		if base_model.diffusion is not None:
			diffusion_named_params=list(base_model.diffusion.named_parameters())
			for(name,p)in diffusion_named_params:
				if'net1'in name or'net2'in name:matrix_params.append(p)
