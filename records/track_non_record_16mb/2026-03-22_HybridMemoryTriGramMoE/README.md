...
I also ran the script end-to-end in this environment with a **tiny CPU-only smoke configuration** using a synthetic dataset and tokenizer. This is only a functional check, not a meaningful quality benchmark. The run completed 1 training step and 2 validation passes successfully, producing a final `val_bpb` of `4.2172` on the toy dataset. See `smoke_cpu_train.log` for the captured output.
